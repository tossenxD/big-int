...
This section is structured as follows: In section \ref{subsec:addalg} we present
an algorithm to compute big integer addition in parallel. In sections
\ref{subsec:addcud} and \ref{subsec:addfut} we discuss how to efficiently
implement the algorithm in CUDA and Futhark, respectively. Lastly, in section
\ref{subsec:sub} we show how to define subtraction from the addition algorithm.
...
$O(1)$. Therefore. the \fun{map}s exhibit work $O(m)$ and span $O(1)$, and the
\fun{scan} work $O(m)$ and span $O(\log m)$. Hence, the total work is $O(m)$ and
the span $O(\log m)$.
...
parallelism in excess: Since the operation runs in sublinear span (with two of
...
handle all input sizes genericly rather than relying on multiple hand-written
...
a CUDA block, we use a the rule $(a + b - 1) / b = \lceil a / b \rceil$ to round up to
...
\item Each warp scans it's elements in $\log \mathit{warpsize}$ iterations.
\item The end-of-warp results are put in the first warp, synchronized, and then
  scanned.
\item The results of ``II'' are fetched, synchronized, and distributed over the
  results ``I''.
\end{enumerate}
...
Using this idea, we are now able to write all three versions. For abbrivety, we
...
\lstinputlisting[language=CPP,firstline=212,lastline=236,caption={\footnotesize
  CUDA \textit{badd} \texttt{V3} implementation body from file
  \texttt{ker-add.cu.h}, where registers \texttt{ass}, \texttt{bss}, and
  ...
  buffer
  \texttt{shmem}.},label={addopcuda},firstnumber=212]{../cuda/ker-add.cu.h}
...
(simple abstract functions that run comparatibly well), versions \texttt{V2} and
...
\texttt{us} and \texttt{vs} of size $\mathtt{ipb}\cdot \mathtt{m}$ from global to shared memory utilizing array short-circuiting (from file \texttt{add.fut} slightly
...
\textit{badd} algorithm). For abbrevity, we focus on \texttt{V3}, as they are
...
\begin{lstlisting}[language=futhark,firstnumber=130,gobble=2,caption={\footnotesize Futhark \textit{badd} \texttt{V3} main function body using base \texttt{ui} and carry type \texttt{ct} from file \texttt{add.fut}.},label={futbaddv3}]
...
While \texttt{V2} and \texttt{V3} are still somewhat succint, it is clear that
...
and thus, making it easy to follow and reason about the semantics.
...
interrested in the difference, we can check which input is the largest (by a
