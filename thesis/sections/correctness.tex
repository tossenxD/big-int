...
(A final note on validating division in C; the divisor use a randomly generated
...
(multiplication). \uuline{All tests validates}.
...
respectively. \uuline{All tests for addition, subtraction, and multiplication
  validates -- division is undecided}.

{\red TODO there is a bug in the tests for division and subtraction in bases
  other than 2$^{32}$, so don't know if division validates or not right now}
...
\ref{sec:per}. \uuline{All tests validate}.
