...
void run_tests(uint32_t m_init, int m_grwth, uint32_t runs, struct futhark_context *ctx) {
    ...
    for (uint32_t i=0, m=m_init; i < runs;
        ...
        uint32_t *v = (uint32_t *) malloc(m * sizeof(uint32_t));

        rand_bigint(m, rand_uint32_t() % m + 1, u);
        rand_bigint(m, rand_uint32_t() % m + 1, v);
        ...
        struct futhark_u64_1d *u_fut_64 = futhark_new_u64_1d(ctx, u_64, m/2);
        struct futhark_u64_1d *v_fut_64 = futhark_new_u64_1d(ctx, v_64, m/2);
        futhark_context_sync(ctx);

        // allocate output arrays
        uint32_t *w_gmp_add = (uint32_t *) malloc((m+1) * sizeof(uint32_t));
        uint32_t *w_gmp_mul = (uint32_t *) malloc((m*2) * sizeof(uint32_t));
        uint32_t *w_gmp_div = (uint32_t *) malloc(m * sizeof(uint32_t));

        uint32_t *w_our = (uint32_t *) malloc(m * sizeof(uint32_t));
        for (uint32_t j=0; j<m; j++) { w_our[j] = 0; }
        ...
        struct futhark_u64_1d *w_fut_add_64 = futhark_new_u64_1d(ctx, w_our_64, m/2);
        struct futhark_u64_1d *w_fut_mul_64 = futhark_new_u64_1d(ctx, w_our_64, m/2);
        struct futhark_u64_1d *w_fut_div_64 = futhark_new_u64_1d(ctx, w_our_64, m/2);
        futhark_context_sync(ctx);
